...
def test_log_jsonl_mode_overwrite(tmp_path: Path, monkeypatch) -> None:
    scripts_root = tmp_path / "scripts"
    output_root = tmp_path / "output"
    data_root = tmp_path / "data"
    log_path = output_root / "performance" / "pipeline_runs.jsonl"
    ...
    records = _parse_jsonl(log_path)
    config_count = sum(1 for rec in records if rec.get("status") == "CONFIG")
