...
def write_hardware_date_recommendation(export_dir: Path) -> Path:
    ...
    lines = [
        ...
        "## Statistical Context",
        "",
        "- RTL-SDR -> Airspy's huge difference obscures the evaluation of subsequent cable/adapter/parameter differences.",
        " Limited evaluation after Airspy is required.",
        "- 2026-01-14 is adopted as the boundary of the main analysis to stably express the structural changes due to hardware updates.",
        "- 2026-01-10 is adopted as the sensitivity analysis boundary to check the robustness when including the effect of the transition period.",
        "- Adapter comparison uses 2026-03-01 as the start date, and 2026-02-28 is treated as a mixed day and is not included in the main conclusion.",
        "- Cable v2 is only used as a reference for comparing candidates and is not used as a basis for the main conclusion.",
        "",
        "## Practical Use",
        "",
        "- The main analysis is fixed at the 2026-01-14 boundary, and the 2026-01-10 sensitivity analysis is also included in a separate section.",
        "- Include the post-Airspy limited subset in the standard report and compare micro-differences there.",
    ]
...
def write_needed_files_for_statistics(export_dir: Path, candidates: list[AICandidateFile]) -> Path:
    ...
    lines = [
        ...
        "",
        "This is a list of candidate files necessary for future statistical judgments.",
        "",
    ]
...
def write_ai_settings_snapshot(export_dir: Path, generated_at: str) -> tuple[Path, Path]:
    ...
    md_lines = [
        ...
        "",
        "- `adsb_detect_change_point.py` and `adsb_detect_multi_change_points.py` are ",
        " Call `arena.lib.config.get_quality_thresholds()` to get the thresholds.",
        "- `get_quality_thresholds()` comes from `settings.toml` (`scripts/config/settings.toml`)",
        "See `quality.min_auc_n_used` and `quality.min_minutes_covered`.",
        "- Therefore, the quality threshold for change point analysis is managed in `settings.toml`.",
        "",
...
def write_ai_change_point_note(export_dir: Path, settings_path: str) -> Path:
    ...
    lines = [
        ...
        "",
        "Reference for change point analysis-related settings included in this export:",
        "",
        "- script:",
        ...
        "",
        "Note:",
        "- The script does not read `settings.toml` directly, but instead ",
        "Indirectly referenced via `get_quality_thresholds()`.",
    ]
