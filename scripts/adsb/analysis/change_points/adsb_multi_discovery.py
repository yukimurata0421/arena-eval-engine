...
def run_clean_discovery():
    ...
    log.info(f">>> Exclusion: first day ({min_date.strftime('%Y-%m-%d')})  and last day ({max_date.strftime('%Y-%m-%d')})  will be excluded.")
    ...
    log.info("\n" + "="*80)
    log.info(f"{'Phase':<10} | {'Estimated start':<12} | {'Avg aircraft/day (est.)':<20} | {'Improvement'}")
    log.info("-" * 80)
    ...
    for i in range(len(unique_indices) + 1):
        ...
        log.info(f"Phase {i+1:<2} | {start_date.strftime('%Y-%m-%d'):<12} | {int(phase_mean):>15,} aircraft | {imp_str}")
    log.info("="*80)
