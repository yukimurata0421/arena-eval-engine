...
def _build_report_text(payload: dict[str, Any]) -> str:
    ...
    cps = payload.get("detected_change_points", [])
    if cps:
        for cp in cps:
            lines.append(
                "- cp{rank}: index={index} date={date} confidence_pct={confidence_pct:.2f}".format(
                    rank=cp.get("rank"),
                    index=cp.get("index"),
                    date=cp.get("date"),
    ...
    segments = payload.get("summary_before_after", [])
    if segments:
        for seg in segments:
            lines.append(
                "- segment{segment_index}: n={n} mean={mean} median={median} "
                "range=[{date_from}..{date_to}] index=[{start_index},{end_index_exclusive})".format(**seg)
            )
    else:
        lines.append("- (none)")
...
def run_multi_discovery_analysis() -> None:
    ...
    report_path = out_dir / "multi_change_points_report.txt"
    json_path = out_dir / "multi_change_points_result.json"
    plot_path = out_dir / "multi_change_points_histogram.png"
