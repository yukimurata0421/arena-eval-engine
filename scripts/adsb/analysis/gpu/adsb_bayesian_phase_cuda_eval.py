...
def run_bayesian_phase_cuda_analysis():
    ...
    print(f" ADS-B Bayesian evaluation (CPU parallel {n_cores}cores / NumPyro backend)")
    ...
    print("\n" + "=" * 70)
    print("  ADS-B Bayesian Report (CPU parallel / PyMC+NumPyro)")
    print("=" * 70)
    ...
    for i in range(1, num_phases):
        ...
        print(
            ...
            f"[{hdi[0]:>+7.1f}, {hdi[1]:>+7.1f}]  {prob_positive:>6.1f}%"
        )
        results.append({
            "comparison": label,
