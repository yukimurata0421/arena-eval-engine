...
def run_bayesian_analysis():
    ...
    for i in range(1, num_phases):
        ...
        if i > 1:
            ...
            print(f"{label_prev:<35} {mean_prev:>+7.1f}% [{hdi_prev_lo:>+7.1f}, {hdi_prev_hi:>+7.1f}]  {prob_prev:>6.1f}%")
    ...
    print("\n--- covariate effect ---")
    beta_traffic_hdi = hdi_bounds(beta_traffic_samples, hdi_prob=0.94)
    print(f"  Traffic elasticity: {np.mean(beta_traffic_samples):.4f} (94% HDI: {beta_traffic_hdi})")
